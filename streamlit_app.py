...
def main():
    ...
    if missing_secrets:
        ...
        st.warning("Please add the missing secrets to `.streamlit/secrets.toml`.")
        st.info("Required: GOOGLE_API_KEY, client_id, client_secret, redirect_uri")
        st.stop()
...
# =============== AGENT LOOP (DEPRECATED - Gemini handles this natively) ===============
# The RAG pipeline now uses Gemini's built-in function calling.
# This stub is kept for compatibility but is no longer called.
...
def run_agent_loop(user_question: str, conversation_history: list, rag_pipeline, llm_client, llm_type: str, model_deployment: str = None) -> tuple:
    """
    DEPRECATED: Gemini API now handles function calling natively inside rag_pipeline.query().
    This stub exists only for backward compatibility.
    """
    raise NotImplementedError("run_agent_loop is deprecated. Use rag_pipeline.query() directly.")
            ...
            if "references" in msg and msg["references"]:
                ...
                for j, doc in enumerate(msg["references"][:3], 1):
                    if isinstance(doc, dict):
                        ...
                        meta = doc.get("metadata", {})
                        src = clean_text_for_pdf(meta.get("source", "Unknown"))
                        page = meta.get("page", "?")
